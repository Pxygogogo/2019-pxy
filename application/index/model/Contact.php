...
class Contact extends Model
{
    ...
    public function addContact($input)
    {
        ...
        $validate = new Validate([
            'name' => 'require',
            'salutation' => 'require',
            'mobile_phone' => 'require|length:11',
            'telephone' => 'require',
            'fax' => 'require',
            'email' => 'require|email',
        ], [
            'name.require' => '请输入联系人名字！',
            'salutation.require' => '请输入联系人称谓!',
            'mobile_phone.require' => '请输入联系人手机!',
            'mobile_phone.length' => '联系人手机为11位!',
    ...
    public function searchBy($input)
    {
        ...
        try {
            ...
            $salutationRes = Db::table('contact')->where('salutation', 'like', '%' . $keywords . '%')->select();
            $mobile_phoneRes = Db::table('contact')->where('mobile_phone', 'like', '%' . $keywords . '%')->select();
            ...
            $client_idRes = Db::table('contact')->where('client_id', 'like', '%' . $keywords . '%')->select();
            $client_nameRes = Db::table('contact')->where('client_name', 'like', '%' . $keywords . '%')->select();
        } catch (DataNotFoundException $e) {
        } catch (ModelNotFoundException $e) {
        } catch (DbException $e) {
