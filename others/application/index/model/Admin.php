...
class Admin extends Model
{
    ...
    public function pass($input)
    {
        ...
        $userinfo = null;
        try {
            $userinfo = Db::table('user')->where('user_id', session('user_id'))->where('password', md5($input['password']))->find();
        ...
        $res = null;
        try {
            $res = Db::table('user')->where("user_id", $userinfo['user_id'])->update(['password' => md5($input['new_pass'])]);
