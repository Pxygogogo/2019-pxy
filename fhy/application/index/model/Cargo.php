...
//        halt($input);
        ...
        $validate = new Validate([
            ...
            'temp' => 'require',
            'deadline' => 'require|number',
        ], [
            'name.require' => '请输入货物名称！',
            'kind.require' => '请输入货物类型！',
            'pack.require' => '请输入包装方式！',
            'unit.require' => '请输入计量单位！',
            'length.require' => '请输入计量长度！',
            'length.number' => '计量长度为数字！',
            'width.require' => '请输入计量宽度！',
            'width.number' => '计量宽度为数字！',
            'height.require' => '亲输入计量高度！',
            'height.number' => '计量高度为数字！',
            'weight.require' => '亲输入计量重量!',
            'weight.number' => '计量重量为数字!',
            'temp.require' => '请输入保鲜温度！',
            'deadline.require' => '请输入保质期！',
            'deadline.number' => '保质期为数字！',
        ]);
        $dt = [
            'name' => $input['name'],
            'kind' => $input['kind'],
...
//        halt($input);
    ...
    public function searchBy($input)
    {
        ...
        try {
            ...
            $typeRes = Db::table('cargo')->where('kind', 'like', '%' . $keywords . '%')->select();
            $package_wayRes = Db::table('cargo')->where('pack', 'like', '%' . $keywords . '%')->select();
            $measure_unitRes = Db::table('cargo')->where('unit', 'like', '%' . $keywords . '%')->select();
            $lengthRes = Db::table('cargo')->where('length', 'like', '%' . $keywords . '%')->select();
            $widthRes = Db::table('cargo')->where('width', 'like', '%' . $keywords . '%')->select();
            $heightRes = Db::table('cargo')->where('height', 'like', '%' . $keywords . '%')->select();
            $weightRes = Db::table('cargo')->where('weight', 'like', '%' . $keywords . '%')->select();
            $temperatureRes = Db::table('cargo')->where('temp', 'like', '%' . $keywords . '%')->select();
            $expiration_dateRes = Db::table('cargo')->where('deadline', 'like', '%' . $keywords . '%')->select();
        } catch (DataNotFoundException $e) {
        } catch (ModelNotFoundException $e) {
        } catch (DbException $e) {
